...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER"  --dbname "$DB_NAME" <<-EOSQL
  BEGIN;
    ALTER ROLE $API_USER SET search_path = nugget,nugget_api,public;
    ...
    GRANT ALL ON ALL SEQUENCES IN SCHEMA nugget TO $API_USER;
    GRANT ALL ON ALL TABLES IN SCHEMA nugget TO $API_USER;
    GRANT ALL ON ALL FUNCTIONS IN SCHEMA nugget TO $API_USER;

    ALTER SCHEMA nugget_api OWNER TO $API_USER;
    ALTER SCHEMA nugget OWNER TO $API_USER;
    ...
    GRANT USAGE ON SCHEMA nugget TO $SUPERTOKENS_USER;
    GRANT ALL ON ALL SEQUENCES IN SCHEMA nugget TO $SUPERTOKENS_USER;
    GRANT ALL ON ALL TABLES IN SCHEMA nugget TO $SUPERTOKENS_USER;
